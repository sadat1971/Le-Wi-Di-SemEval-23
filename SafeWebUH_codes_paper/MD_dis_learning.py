...
# Load a process the data, convert to pickle
train = pd.read_json(path + "data_practicephase_cleardev/data_practicephase_cleardev/MD-Agreement_dataset/MD-Agreement_train.json", orient='index')
dev = pd.read_json(path + "data_practicephase_cleardev/data_practicephase_cleardev/MD-Agreement_dataset/MD-Agreement_dev.json", orient='index')
test = pd.read_json(path + "data_post-competition/data_post-competition/MD-Agreement_dataset/MD-Agreement_test.json", orient='index')
train.to_pickle(path + "Data/MD_train.pkl")
dev.to_pickle(path + "Data/MD_dev.pkl")
test.to_pickle(path + "MD_v2/Data/MD_test.pkl")




def tokenization_for_BERT(df, path=" /path/to/data/MD_v2/Data/", filename="put_the_filename_here", saveit="No"):
...
class Bertregressor(nn.Module):
    ...
    def __init__(self, hidden_size=50, dropout=0):
        ...
        # We'd like to build a fully connected neural network
...
def initialize_model(epochs, train_dataloader, device, H, D_in=768, dropout=0.25, classes=2):
    """Initialize the Bert, the optimizer and the learning rate scheduler.
    """
    # Instantiate Bert regressor
    bert_regressor = Bertregressor(hidden_size=H, dropout=dropout)
...
parser.add_argument("--data_dir",
                    type=str,
                    default="/path/to/data/MD_v2/Data/",
                     help="Input data path.")
parser.add_argument("--log_dir",
                     type=str,
                     default="/path/to/data/MD_v2/Results/SafeWebUH/",
...
train = tokenization_for_BERT(train, path=args.data_dir, filename= "MD_train.pkl" , saveit="Yes")
