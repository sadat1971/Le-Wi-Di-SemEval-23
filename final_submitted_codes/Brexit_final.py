...
def train_model(model, train_dataloader,val_dataloader, epochs, evaluation, device, optimizer, scheduler):
    ...
    for epoch_i in range(epochs):
        ...
        df.to_pickle(args.log_dir + "/temp/testresult"  + ".pkl")
...
def compute_reg_score(soft, hard, probs, agg, off, wt, save_result=True, produce_result=True):
    other_score = np.array(agg)*0.58299109 + np.array(off)*0.33627756 - 0.0023297653922081385
    ...
    if save_result:
        ...
        record_results.to_csv("/media2/special/Sadat/Brexit_v2/Result/" + "HS-Brexit_results.tsv", sep='\t', header=False, index=False)
    if produce_result:
...
df = compute_reg_score(soft, hard, probs, agg, off, 1, True, False)
