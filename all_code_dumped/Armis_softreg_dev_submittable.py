...
def tokenization_for_BERT(df, path="/media2/special/Sadat/Armis/Data/", filename="put_the_filename_here", saveit="No"):
...
class BertClassifier(nn.Module):
    ...
    def __init__(self, hidden_size=50, dropout=0): 
        ...
        D_in, H, D_out = 1024, hidden_size, 1 #Just one hidden layer with 50 units in it
...
def train_model(model, train_dataloader,val_dataloader, epochs, evaluation, device, optimizer, scheduler):
    ...
    for epoch_i in range(epochs):
        ...
        if evaluation == True:
            ...
            val_loss, val_accuracy, f1, df, ce = evaluate(model, val_dataloader, device)
        ...
        val_loss, val_accuracy, val_f1sc, df, ce = evaluate(model, val_dataloader, device)
        print("VAL LOSS IS {}, ce is {}, f1 is {}".format(val_loss, ce, val_f1sc))
        ...
        with open(args.log_dir+ "/" + str(args.version) + "_specialcheck_BERT_soft_results.txt", 'a') as r:
...
def evaluate(model, dataloader, device):
    ...
    T_disc = [0 if t<.5 else 1 for t in T]
    P_disc = [0 if t<.5 else 1 for t in P]
    f1 = f1_score(T_disc, P_disc, average='micro')
    acc = accuracy_score(T_disc, P_disc)
    df = pd.DataFrame()
    df["GT"] = all_labels
    df["probs"] = P
    ...
    record_results.to_csv(args.log_dir + "Armis.tsv", sep='\t', header=False, index=False)
    return avg_loss, acc, f1, df, ce
...
parser.add_argument("--data_dir", 
                    type=str, 
                    default="/media2/special/Sadat/Armis/Data/",
                     help="Input data path.")
parser.add_argument("--log_dir",
                     type=str, 
                     default="/media2/special/Sadat/Armis/Result/",
                     help="Store result path.")
parser.add_argument("--model_path_dir",
                     type=str, 
                     default="/media2/special/Sadat/Armis/Data/Result/",
                     help="Store result path.")
...
parser.add_argument("--version", type=int, default=0, help="what is the version?")
...
with open(args.log_dir+ "/" + str(args.version) + "_specialcheck_BERT_soft_results.txt", 'a') as r:
    stuffs = "bsize: " + str(args.batch_size) + "\n" + "dropout: " + str(args.dropout) + "\nhidden:" + str(args.hidden_size)
    r.write(stuffs)
    r.write("\n")
...
test = pd.read_pickle(args.data_dir + "Armis_dev.pkl")
test = tokenization_for_BERT(test, path=args.data_dir, filename="Armis_dev" , saveit="Yes")
