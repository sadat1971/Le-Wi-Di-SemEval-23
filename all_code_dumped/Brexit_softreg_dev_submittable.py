...
def train_model(model, train_dataloader,val_dataloader, epochs, evaluation, device, optimizer, scheduler):
    ...
    for epoch_i in range(epochs):
        ...
        if val_loss<best_loss:
            best_loss = val_loss
            df.to_pickle(args.log_dir + "/temp/valresult"  + ".pkl")
...
# test =pd.read_pickle(args.data_dir + "Brexit_dev.pkl")
...
#     preds.append(annot)
# #####
# pred_wise, prob_wise, f1m, f1b = compute_aggregated_performance(preds, probs_otherinfo, soft_lab, test.hard_label.tolist())
# print("For weight {}, f1 micro is {:.4f}, f1 binary is {:.4f}  the CE loss is softmax score wise is {:.4f} and average prediction wise {:.4f}".format(wt, f1m, f1b, prob_wise,pred_wise))