...
def train_model(model, train_dataloader,val_dataloader, epochs, evaluation, device, optimizer, scheduler):
    ...
    for epoch_i in range(epochs):
        ...
        ce, f1, val_loss = evaluate(model, val_dataloader, device)
        ...
        with open(args.log_dir+ "/" + str(args.ver) + "_special_BERT_MSEloss.txt", 'a') as r:
...
def evaluate(model, dataloader, device):
    ...
    record_results.to_csv(args.log_dir + "mdresults.tsv", sep='\t', header=False, index=False)

    return ce, f1*100, avg_loss
